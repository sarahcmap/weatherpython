...
conn = psycopg2.connect(dbname="rtdap", user="cmap", password="cm@pcm@p", host="10.10.1.39", port="5432")
...
cur.execute("""SELECT * FROM information_schema.columns
       WHERE table_schema = 'public'
       AND table_name = 'NwsObservation_2018'""")
for table in cur.fetchall():
    print(table)

# weather stuff is 'NwsObservation_2019' and 'NwsObservation_2018'
with psycopg2.connect(dbname="rtdap", user="cmap", password="cm@pcm@p", host="10.10.1.39", port="5432") as conn:
    with conn.cursor(name = 'testcursor') as cursor:
        cursor.itersize = 20000
        tablename = "\"NwsObservation_2018" + "\""
        query = """COPY
        (SELECT *
            FROM %s)
...
# execute query and save to file
        with open(
            "C:/Users/sbuchhorn/Desktop/hhts/weather/NWS_18.csv", "w") as file:
